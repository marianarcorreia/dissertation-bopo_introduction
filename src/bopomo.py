...
class ActorModel(torch.nn.Module):
    ...
    def forward(self, data: HeteroData):
        ...
        #concatena os embeddings da maq., features da aresta e emb. da operação
...
class Policy(nn.Module):
    ...
    def act(self, state, sample, num):
        ...
        _dbg(2, f"  act() | step={num} | sample_mode={sample} | chosen_edge={int(action)} | logprob={float(action_logprob):.4f} | n_valid={int((~state[('machine','exec','operation')].mask).sum())}")
    ...
    def act_batch(self, batched_state, greedy_flags=None):
        ...
        row, _ = batched_state[('machine', 'exec', 'operation')].edge_index
        batch_index = batched_state["machine"].batch[row]
        mask = batched_state[('machine', 'exec', 'operation')].mask

        num_graphs = int(batched_state["operation"].batch.max().item()) + 1
        actions = []
        logprobs = []
        entropies = []
        valid_counts = []
