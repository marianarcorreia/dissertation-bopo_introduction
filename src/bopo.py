...
from src.bopo_utils import (bopo_group_loss, summarize_action_entropy, compute_actor_grad_norm,
                             masked_chosen_logprobs, memory_efficient_update)
...
class Policy(nn.Module):
    ...
    def chosen_logprobs(self, batched_state, actions):
        ...
        index among graph g's own ('machine','exec','job') edges."""
        store = batched_state[('machine', 'exec', 'job')]
        graph = batched_state["machine"].batch[store.edge_index[0]]
        return masked_chosen_logprobs(self.actor(batched_state).T[0], graph, store.mask, actions)
...
class BOPO:
    def __init__(self, lr, env, metadata, hidden_channels=128, num_layers=2, heads=3,
        ...
        # (verified to float32 rounding) but ~2.5 GB instead of 13-23 GB on the blocking
        # instances; False restores the original single-pass update (see _update_memory_efficient)
    ...
    def sample_group(self, instance_index, record=False):
        """record=False: log-probs keep their autograd graph (standard update).
        ...
        all_entropies = []
        all_valid_counts = []
        trajectories: List[list] = [[] for _ in range(self.B)]
    ...
    def _update_memory_efficient(self, instance_index):
