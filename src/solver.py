...
def solve_fjsp(jobs, operations, info, time_limit=15):
    jobs_or = []
    for job in jobs:
        job_info = []
        for o_id in job:
    ...
    return flexible_jobshop(jobs_or, info["machinesNb"], time_limit)


def flexible_jobshop(jobs, num_machines, time_limit=15):
    ...
    for job_id in all_jobs:
        ...
        for task_id in range(num_tasks):
            ...
            duration = model.new_int_var(min_duration, max_duration, "duration" + suffix_name)
            end = model.new_int_var(0, horizon, "end" + suffix_name)
            interval = model.new_interval_var(start, duration, end, "interval" + suffix_name)
    ...
    return solver.status_name(status), solver.objective_value, list_dicts
